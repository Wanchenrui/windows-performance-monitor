...
当前版本为 `0.2.0`：一个可信化后的 Windows 10/11 本地性能监控原型。
它仍不是商业成品，但已修复 0.1 版的主要指标、采样与运行问题。
...
- RAM 中的历史记录按真实时间裁剪，默认保留最近 60 分钟。
...
会话再次启动时（即使请求了不同端口）会通过命名互斥体和瞬态实例文件识别
并打开现有页面。若目标端口被其他程序占用，程序会显示明确错误并以退出码
`2` 结束。
...
- `GET /api/stats`：原子指标快照、采样序号、采样时间、数据年龄、历史窗口、
  调度抖动、累计缺口和采集错误。
- `GET /api/health`：实例标识、版本和简要健康状态，用于单实例探测。

API 版本为 `0.2`。后续拆分原生代理与桌面客户端时应保持版本化兼容策略。
...
测试覆盖进程 CPU 公式、指标单位与来源、真实时间历史裁剪、陈旧状态、
并发快照一致性、固定周期缺口计算和本地 HTTP 安全响应头。
...
生成 `dist\perf-monitor.exe`，验证版本资源，并输出带 SHA-256 的
`dist\build-manifest.json`。这里的“可复现”指固定源代码、Python 3.12 和
锁定依赖可重复生成可运行产物，不承诺二进制逐字节相同。
...
- 0.1 的旧 EXE 在首次 0.2 构建前应另存为
  `dist\perf-monitor-0.1-legacy.exe`，用于运行级回退。
- 当前仓库尚无首个 Git 提交。提交 0.2 前应先人工审阅变更；之后每个功能
  批次使用独立提交和版本标签。
